...
async def list_brands(db: AsyncSession = Depends(get_db)):
    ...
    for brand_name, results in brand_map.items():
        ...
        pos_vals = [r.sentiment_distribution.get("positive", 0) for r in results if r.sentiment_distribution]
        neg_vals = [r.sentiment_distribution.get("negative", 0) for r in results if r.sentiment_distribution]
        neu_vals = [r.sentiment_distribution.get("neutral", 0) for r in results if r.sentiment_distribution]
        ...
        summaries.append(BrandSummary(
            ...
            latest_analysis_at=latest_job.completed_at.isoformat() if latest_job and latest_job.completed_at else None,
...
async def compare_brands(
    ...
    if len(brand_names) < 2:
        raise HTTPException(status_code=400, detail="Provide at least 2 brands to compare")
...
async def get_brand_history(
    ...
    if not results:
        raise HTTPException(
            status_code=404,
            detail=f"No analyses found for brand '{brand_name}'"
        )
    ...
    for r in results:
        ...
        history.append({
            ...
            "analyzed_at":        job.completed_at.isoformat() if job and job.completed_at else None,
        })
