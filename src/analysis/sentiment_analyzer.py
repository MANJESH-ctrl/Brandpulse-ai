...
class SentimentAnalyzer:
    ...
    async def analyze(self, posts: List[Dict], brand_name: str) -> Dict:
        ...
        primary = (
            f"{self.PRIMARY_MODEL} (cerebras)"
            if self._cerebras
            else f"{self.FALLBACK_MODEL} (groq)"
        )
        logger.info(
        ...
        async def run(batch_data):
            async with semaphore:
                return await self._batch_with_fallback(
                    *batch_data, brand_name=brand_name
                )
            ...
            for attempt in range(4):
                try:
                    ...
                    if "429" in err and attempt < 3:
                        ...
                        logger.warning(
                            "cerebras_rate_limit_retry", attempt=attempt + 1, wait=wait
                        )
        ...
        for attempt in range(4):
            try:
                ...
                if "429" in err and attempt < 3:
                    wait = (2**attempt) * 3
                    logger.warning(
                        "groq_rate_limit_retry", attempt=attempt + 1, wait=wait
                    )
                    await asyncio.sleep(wait)
                    continue
