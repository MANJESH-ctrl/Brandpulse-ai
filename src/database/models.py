...
class AnalysisResult(Base):
    ...
    brand_name = Column(String, nullable=False, index=True)
    sentiment_distribution = Column(JSON)      # {positive: 0.4, negative: 0.3, neutral: 0.3}
...
class CollectedPost(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    source_id = Column(String, nullable=True, index=True)  # original post id from collector
    ...
    platform_meta = Column(JSON, nullable=True)  # upvotes, views, likes — platform specific
